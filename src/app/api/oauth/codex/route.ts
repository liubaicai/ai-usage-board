...
async function formPost(url: string, body: URLSearchParams): Promise<{ status: number; data: Record<string, unknown> }> {
...
export async function POST(req: Request) {
  try {
    ...
    // 轮询授权结果
    if (body.deviceCode) {
      const r = await formPost("https://auth.openai.com/oauth/device/token", new URLSearchParams({
        client_id: CLIENT_ID,
    ...
    // 发起设备授权
    const r = await formPost("https://auth.openai.com/oauth/device/code", new URLSearchParams({
      client_id: CLIENT_ID,
